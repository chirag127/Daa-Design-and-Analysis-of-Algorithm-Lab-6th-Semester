...
This code implements Kruskal's algorithm to find the minimum cost spanning tree of a connected undirected graph. The input consists of the number of nodes `n` and edges `m` in the graph, followed by `m` lines each containing `x`, `y`, and `z` representing an edge between node `x` and node `y` with weight `z`. 
